    ...
    try {
      const response = await fetch('https://aifounderblueprint.com.au/create-checkout-session', {
        ...
        body: JSON.stringify({
          ...
          amount: submittedPriceObj.amount,
          currency: submittedPriceObj.currency,
          productName: formData.course
        })
      });
      ...
      console.error(error);
      alert("Error contacting payment server. Is the backend running?");
      setIsRedirecting(false);
    }
  };
    ...
    return (
      <div style={{ backgroundColor: 'var(--bg-main)', minHeight: '80vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '48px 24px' }}>
        <Card style={{ backgroundColor: 'var(--color-secondary-light)', textAlign: 'center', padding: '48px', maxWidth: '600px' }}>
          ...
          <Button onClick={handlePayment} disabled={isRedirecting} variant="primary" style={{ padding: '16px 32px', fontSize: '1.25rem', width: '100%', opacity: isRedirecting ? 0.7 : 1 }}>
            {isRedirecting ? (
